...
async function syncCloud() { if (!isSupabaseConfigured || syncInFlight) return; syncInFlight = true; syncError.value = ''; try { cloudUserId ??= (await ensureAnonymousSession()).user.id; const queue = createSyncQueue(window.localStorage, cloudUserId); if (queue.pending().some((entry) => entry.userId === cloudUserId)) queue.markAttempt(cloudUserId, '正在重试同步'); const merged = await pullLearningState(supabase, cloudUserId, learning.state); learning.replaceState(merged); await syncLearningState(supabase, cloudUserId, merged); queue.remove(cloudUserId); pendingSyncCount.value = 0; cloudStatus.value = '已同步' } catch (error) { console.warn('[Learning] 云同步不可用，继续使用本地数据', error); syncError.value = error instanceof Error ? error.message : '同步失败'; if (cloudUserId) { const queue = createSyncQueue(window.localStorage, cloudUserId); queue.enqueue(cloudUserId, syncError.value); pendingSyncCount.value = queue.pending().length; cloudStatus.value = `离线模式 · ${pendingSyncCount.value} 待同步` } else cloudStatus.value = '离线模式' } finally { syncInFlight = false } }
...
<template>
  <div class="app-shell">
    ...
    <main class="main-content"><header class="topbar"><div><p class="eyebrow">HEHE READING ROOM</p><h1>{{ activeTitle }}</h1></div><div class="header-stat"><strong>{{ learning.wordbookCount }}</strong><span>生词</span></div></header>
      ...
      <ProfileView v-else :mastered="learning.state.masteredWordIds.length" :wordbook="learning.wordbookCount" :completed="learning.completedCount" :total-chapters="totalChapters" :study-seconds="learning.state.studySeconds" :study-days="learning.state.studyDates.length" :preferences="learning.state.preferences" :history="readingHistoryEntries" :study-sessions="learning.state.studySessions" :review-records="learning.state.reviewRecords" :custom-book-count="customBooks.length" :sync-status="cloudStatus" :pending-sync-count="pendingSyncCount" :sync-error="syncError" @navigate="activeTab = $event" @open-history="openHistory" @clear-history="learning.clearReadingHistory" @update-preferences="updateReaderPreferences" @export-data="exportData" @import-data="triggerBackupImport" @clear-data="clearLocalData" @retry-sync="syncCloud" />
    </main>
