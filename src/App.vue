...
import PronunciationControls from './components/PronunciationControls.vue'
...
import type { LearningState, ReadingPreferences } from './stores/learning'
...
const pendingLocalState = ref<LearningState | null>(null)
const pendingCloudState = ref<LearningState | null>(null)
...
async function syncCloud(resolution?: 'local' | 'cloud') {
  if (!isSupabaseConfigured || syncInFlight) return
  syncInFlight = true
  syncError.value = ''
  try {
    cloudUserId ??= (await ensureAnonymousSession()).user.id
    const queue = createSyncQueue(window.localStorage, cloudUserId)
    if (queue.pending().some((entry) => entry.userId === cloudUserId)) queue.markAttempt(cloudUserId, '正在重试同步')
    const localState = pendingLocalState.value ?? JSON.parse(JSON.stringify(learning.state)) as LearningState
    const pulled = pendingCloudState.value && resolution ? { state: pendingCloudState.value, conflicts: syncConflicts.value } : await pullLearningState(supabase, cloudUserId, localState)
    if (pulled.conflicts.length && !resolution) {
      pendingLocalState.value = localState
      pendingCloudState.value = pulled.state as LearningState
      syncConflicts.value = pulled.conflicts
      cloudStatus.value = '发现同步冲突'
      return
    }
    const nextState = resolution === 'local' ? localState : pulled.state as LearningState
    learning.replaceState(nextState)
    await syncLearningState(supabase, cloudUserId, nextState)
    pendingLocalState.value = null
    pendingCloudState.value = null
    syncConflicts.value = []
    queue.remove(cloudUserId)
    ...
    if (cloudUserId) {
      const queue = createSyncQueue(window.localStorage, cloudUserId)
      queue.enqueue(cloudUserId, syncError.value)
      pendingSyncCount.value = queue.pending().length
      cloudStatus.value = `离线模式 · ${pendingSyncCount.value} 待同步`
    } else cloudStatus.value = '离线模式'
  } finally {
    syncInFlight = false
  }
}
function resolveSyncConflict(choice: 'local' | 'cloud') { void syncCloud(choice) }
...
<template>
  <div class="app-shell">
    ...
    <main class="main-content"><header class="topbar"><div><p class="eyebrow">HEHE READING ROOM</p><h1>{{ activeTitle }}</h1></div><div class="header-stat"><strong>{{ learning.wordbookCount }}</strong><span>生词</span></div></header>
      ...
      <WordbookView v-else-if="activeTab === 'wordbook'" :words="wordbookWords" :mastered-word-ids="learning.state.masteredWordIds" :review-records="learning.state.reviewRecords" :pronunciation-accent="learning.state.preferences.pronunciationAccent ?? 'us'" @update:accent="updateReaderPreferences({ pronunciationAccent: $event })" @mastery="learning.toggleMastery($event.id)" @review="recordWordReview" @remove="learning.toggleWordbook($event.id)" @shelf="activeTab = 'shelf'" />
      <ProfileView v-else :mastered="learning.state.masteredWordIds.length" :wordbook="learning.wordbookCount" :completed="learning.completedCount" :total-chapters="totalChapters" :study-seconds="learning.state.studySeconds" :study-days="learning.state.studyDates.length" :preferences="learning.state.preferences" :history="readingHistoryEntries" :study-sessions="learning.state.studySessions" :review-records="learning.state.reviewRecords" :custom-book-count="customBooks.length" :sync-status="cloudStatus" :pending-sync-count="pendingSyncCount" :sync-error="syncError" :sync-conflicts="syncConflicts" @navigate="activeTab = $event" @open-history="openHistory" @clear-history="learning.clearReadingHistory" @update-preferences="updateReaderPreferences" @export-data="exportData" @import-data="triggerBackupImport" @clear-data="clearLocalData" @retry-sync="syncCloud" @resolve-conflict="resolveSyncConflict" />
    </main>
