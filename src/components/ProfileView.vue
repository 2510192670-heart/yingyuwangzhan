...
import StudyStatsPanel from './StudyStatsPanel.vue'
...
const props = defineProps<{ mastered: number; wordbook: number; completed: number; totalChapters: number; studySeconds: number; studyDays: number; preferences: ReadingPreferences; history: Array<ReadingHistoryItem & { bookTitle: string; chapterTitle: string }>; studySessions: StudySessionRecord[]; reviewRecords: Record<string, ReviewRecord>; customBookCount: number; syncStatus: string }>()
const emit = defineEmits<{ navigate: [tab: 'shelf' | 'wordbook']; updatePreferences: [next: Partial<ReadingPreferences>]; exportData: []; importData: []; clearData: [] }>()
...
<template>
  <section class="page-section profile-page">
    ...
    <StudyStatsPanel :sessions="studySessions" :review-records="reviewRecords" />
    ...
    <SystemSettingsPanel :custom-book-count="customBookCount" :sync-status="syncStatus" @export="emit('exportData')" @import="emit('importData')" @clear="emit('clearData')" />
  </section>
</template>
