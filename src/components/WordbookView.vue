...
function markReviewMastered() {
  ...
  emit('review', reviewWord.value, 'mastered')
  emit('mastery', reviewWord.value)
  reviewIndex.value += 1
}
function markReviewAgain() {
  if (!reviewWord.value) return
  emit('review', reviewWord.value, 'again')
  reviewIndex.value += 1
}
function exitReview() {
